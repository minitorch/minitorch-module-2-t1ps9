...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return a.f.add_zip(
            a.f.mul_zip(a.f.sigmoid_map(a), grad_output),
            a.f.neg_map(a.f.mul_zip(a.f.mul_zip(a.f.sigmoid_map(a), a.f.sigmoid_map(a)), grad_output))
        )
...
class LT(Function):
    ...
    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        grad_a = zeros(grad_output.shape)
        grad_b = zeros(grad_output.shape)
        return grad_a, grad_b
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        return a._new(a._tensor.permute(*[int(i) for i in order.to_numpy().flatten()]))

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        order = ctx.saved_values[0].to_numpy().flatten().astype(int).tolist()
        reverse_order = list(range(len(order)))
